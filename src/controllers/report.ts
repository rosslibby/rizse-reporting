...
import { DamageDimensions } from 'types'
import { ReportRow } from 'types'
import { Report } from 'types'
import fs from 'fs'
import { dirname } from 'path'

const generator = (data: Report) => {
  ...
  Object.keys(data).forEach((key: string) => {
    const currentItem = data[key as keyof Report]
    if (Array.isArray(currentItem)) {
      ...
      for (let i = 0; i < currentItem.length / pageMax; i++) {
        for (const dataByKey of currentItem.splice(i * pageMax, pageMax)) {
          ...
          Object.keys(dataByKey).forEach((dataKey: string) => {
            const dataItem = dataByKey[dataKey as keyof ReportRow]
            lineItem.push('<span class="column">')
            
            if (typeof dataItem === 'object') {
              Object.keys(dataItem).forEach(iKey => {
                if (typeof dataItem[iKey as keyof DamageDimensions] === 'string') {
                  lineItem.push(`<span class="cell">${dataItem[iKey as keyof DamageDimensions]}</span>`)
                }
              })
...
export const createReport = (req: Request, res: Response) => {
  const { data } = req.body
  const rows: string[] = generator(data)
  const dom = html(data, rows.join('\n'))
  fs.writeFile(dirname('tmp/output.html'), dom, (err) => {
    if (err) {
      console.error(err)
    } else {
      return res.sendFile(dirname('./tmp/output.html'))
    }
  })
  // const template = 
}

const html = (data: Report, rows: string) => {
  return `<html>
    <head>${styles}</head>
    <body>
      <div class="grid">
